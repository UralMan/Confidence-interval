...
1. **Прямой способ** (Forward approach) заключается в генерации множества симуляций на основе предполагаемой модели и последующем сравнении наблюдаемых данных с доверительным интервалом ($CI$) симуляций.   
2. **Обратный способ** (Backward approach) состоит в построении доверительного интервала на основе эмпирических данных и проверке того, насколько симуляционные значения модели попадают в этот коридор.   
...
2. Оценка доверительного интервала по эмпирическим данным   
    ...
    - Классический способ через нормальное приближение: $CI = \left[ \bar{X} - z\_{\alpha/2} \cdot \frac{\sigma}{\sqrt{n}}, \quad \bar{X} + z\_{\alpha/2} \cdot \frac{\sigma}{\sqrt{n}} \right]$   
    - Непараметрический способ через квантильные оценки: $CI = \left[ Q\_{\alpha/2}, \quad Q\_{1-\alpha/2} \right]$   
...
- Симуляции модели:   
    Функция `simulate\_model()`  моделирует генерацию одного значения. В цикле производится 1000 симуляций, результаты которых сохраняются в массиве.   
- Проверка попадания:   
    Определяется доля симуляций, значения которых попадают в интервал \`[lower\\_bound, upper\\_bound]\`.   
- Визуализация:   
